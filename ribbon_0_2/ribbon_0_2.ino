...
void setup() {
  ...
  cbi(ADCSRA,ADPS1) ;
  cbi(ADCSRA,ADPS0) ;
  pinMode(12, OUTPUT); //gate 1
  pinMode(13, OUTPUT); //gate 2


  pinMode(9, OUTPUT); //cv 1
  pinMode(10, OUTPUT); //cv 2
...
void loop()
{
  ...
  if (timer_3 > 100) {
    ...
    Serial.print(lectura_suma,DEC);

    Serial.print("  CV1 PWM: ");
    Serial.println(temporal,DEC);
  ...
  else
  {
    ...
    temporal = 300 - average_l / 2;
    
    temporal = temporal / 20;
    temporal = temporal * 20;
    analogWrite(9,temporal);
  }
